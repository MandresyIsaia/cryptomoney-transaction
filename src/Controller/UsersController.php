...
class UsersController extends AbstractController
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $roleId = $data['role'] ?? 1;
        $role = $this->roleRepository->find($roleId) ?? $this->roleRepository->find(1);

        $gender = $this->genderRepository->find($data['gender']);
    ...
    #[Route('/api/get_session', name: 'get_session_user', methods: ['POST'])]
    public function getSession(SessionInterface $session): JsonResponse
    {
        $userData = $session->get('user');

        if (!$userData) {
            return $this->json(['message' => 'No user data found in session'], 400);
        }
